    ...
    <div class="flex flex-col w-[1160px] site_color_3_background rounded-[15px]">
      <div class="flex flex-col items-center min-h-[290px] pt-[62px] px-[99px]">
        <form class="mb-[56px] flex gap-x-[102px]">
          <div class="flex flex-col items-center">
            ...
            <div class="flex gap-x-[8.5px]">
              <button :class="{ minCount: countComputers === 0 }" type="button"
                @click="countComputers--, calculationPrice()"><img class="h-[23px]"
                  src="@/assets/icon/decrementCount_icon.svg" width="23" height="23"></button>
              <div class="flex justify-center items-center text_8 bg-white rounded-[8px] w-[40px] h-[27px]">
                <p> {{ countComputers }} </p>
              </div>
              <button :class="{ maxCountComputers: countComputers === 20 }" type="button"
                @click="countComputers++, calculationPrice()"><img class="h-[23px]"
                  src="@/assets/icon/incrementCount_icon.svg" width="23" height="23"></button>
            </div>
          </div>
          <div class="flex flex-col items-center">
            ...
            <p class="mb-[5px] text_7">Количество</p>
            <div class="flex gap-x-[8.5px]">
              <button :class="{ minCount: countServers === 0 }" type="button"
                @click="countServers--, calculationPrice()"><img class="h-[23px]"
                  src="@/assets/icon/decrementCount_icon.svg" width="23" height="23"></button>
              <div class="flex justify-center items-center text_8 bg-white rounded-[8px] w-[40px] h-[27px]">
                <p> {{ countServers }} </p>
              </div>
              <button :class="{ maxCountServers: countServers === 2 }" type="button"
                @click="countServers++, calculationPrice()"><img class="h-[23px]"
                  src="@/assets/icon/incrementCount_icon.svg" width="23" height="23"></button>
            </div>
          </div>
          <div>
            ...
            <fieldset class="flex gap-x-[15px] calculator_button input_display text_8">
              <div>
                ...
                <input type="radio" name="answer" id="answer1" @click="radios='7', calculationPrice()"><label
                  for="answer1">ПН-вск</label>
              </div>
              <div>
                <p class="mb-[8px] text-center text_10">рабочее время</p>
                <input type="radio" name="answer" id="answer2" @click="radios='5', calculationPrice()"><label
                  for="answer2">ПН-пт</label>
              </div>
              <div>
                <p class="mb-[20.6px]"></p>
                <input type="radio" name="answer" id="answer3" @click="radios='7/24', calculationPrice()"><label
                  for="answer3">24/7</label>
              </div>
            </fieldset>
          </div>
        </form>
        <div class="flex gap-x-[34px]">
          <div
            class="py-[10px] px-[20px] flex justify-between items-center text_button w-[322px] h-[40px] site_color_7_background rounded-[8px]">
            <p>Итого:</p>
      ...
      <div :class="{ special_offer: countServers === 2 || countComputers === 20 }"
        class="hidden min-h-[289px] flex justify-center items-center gap-x-[94px] site_color_2_background">
        <form class="flex flex-col items-center gap-y-[16px] site_color_1" netlify>
          <p class="mb-[27px] text_1 text-center site_color_7">У вас более 20 компьютеров или 2 серверов? <br>Заполните
            форму и мы свяжемся с Вами</p>
          <div class="calculator_form_input flex justify-center items-center site_color_1">
            <p class="calculator_form_placeholder w-[426px] h-[35px] py-[10px] text-center text_6 absolute site_color_5 rounded-[10px] site_color_7_background"
              :class="{ active: telephone != '' }">Контактный телефон</p>
            <input v-mask="mask" v-model="telephone"
              class="calculator_form_input_text text-center w-[426px] h-[35px] p-[8px] rounded-[10px] text_4" type="tel"
              name="tel" minlength="16" maxlength="16" required>
          </div>
          <UIButton :class="{ notActive: telephone.length != 16 }" variant="personalOffer">Получить персональное
            предложение</UIButton>
        </form>
        <img class="self-end" src="@/assets/pageContentImage/IT-outsoucing_calculator_image.svg" width="384"
          height="258" alt="IT-outsoucing_calculator_image">
      </div>
      <div class="min-h-[394px] px-[218px] pt-[59px] pb-[83px]">
        ...
        <div v-masonry column-width=".grid-sizer">
          <div class="grid-sizer" style="width: calc(100% / 3)"></div>
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
              @click="showDescriptionModalDialog(
              ...
              <img class="h-[13.09px]" src="@/assets/icon/DescriptionButton_icon.svg" width="12"
                height="13.09"></button>
          ...
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
              ...
              <img class="h-[13.09px]" src="@/assets/icon/DescriptionButton_icon.svg" width="12"
                height="13.09"></button>
            <p class="text_9 !text-[12px] !leading-[1.2] w-[182px]">IT - инструменты для службы внутренней безопасности
            </p>
          </div>
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
          ...
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
              ...
              <img class="h-[13.09px]" src="@/assets/icon/DescriptionButton_icon.svg" width="12"
                height="13.09"></button>
            <p class="text_9 !text-[12px] !leading-[1.2] w-[182px]">Персональная настройка системы мониторинга состояния
              (Zabbix)</p>
          </div>
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
          ...
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
              ...
              <img class="h-[13.09px]" src="@/assets/icon/DescriptionButton_icon.svg" width="12"
                height="13.09"></button>
            <p class="text_9 !text-[12px] !leading-[1.2] w-[182px]">По желанию клиента - настройка оповещения об
              инцидентах в Telergam</p>
          </div>
          <div class="flex items-center pb-[11px] gap-x-[6px]" v-masonry-tile>
            <button
              @click="showDescriptionModalDialog(
...
.calculator_form_input:focus-within .calculator_form_input_text,
.active+.calculator_form_input_text {
